...
public:
	...
	int Dequeue()
	{
		// return error value if dequeue operation is performed on empty queue
		if (isEmpty()) {
			return -404;
		}
		// create temp node 
		Node* temp;
		// retrieve data from front/first node of queue
		int dequeuedInt = first->data;
		// assign the temp to the first node
		temp = first;
		// set first node as the next node in queue before deleting (or will be null if this is the only node) 
		first = first->next;
		// delete the temp (old first) node
		delete temp;
		// return the value of the former first node
		return dequeuedInt;
	}
